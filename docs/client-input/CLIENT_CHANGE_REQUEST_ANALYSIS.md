...
| Personal WhatsApp QR                                | Conflicts with official-provider/security requirements                                                                                                       | Prohibited. Use only the official WhatsApp Business Platform / Cloud API                                                                                         | Rejected; no product decision remains        |
...
## 2026-08-08 Phase 4 manual-recording amendment analysis

| Amendment request                         | Existing constraint                                                       | Approved Phase 4 resolution                                                                                                                                          | Boundary                                                                                        |
| ----------------------------------------- | ------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| Attach a manually supplied call recording | Calls must remain on canonical Lead/Contact; no silent/device capture     | Scoped Lead/Contact selection, active `CALL_RECORDING` consent, private signed upload, metadata verification and append-only activity/audit evidence                 | No duplicate customer/call model and no audio in PostgreSQL                                     |
| Identify how a recording was obtained     | Provider and manually supplied material have different provenance         | `call_recordings.source` is `PROVIDER` or `MANUAL_UPLOAD`; manual source records uploader, filename, MIME type, byte length, optional checksum, notes and timestamps | Provider is still authoritative for provider events                                             |
| Let authorized staff listen/download      | Recording is sensitive tenant data                                        | Existing recording-read policy plus Lead assignment scope, consent, retention and short-lived private download URL                                                   | Upload permission is deliberately separate from read/download permission                        |
| Preserve CallerDesk/voice/AI roadmap      | Current phase must not implement future AI or unsafe mobile functionality | One recording abstraction can feed a future approved transcript pipeline                                                                                             | AI transcript/summary, Previous Voice Quick Note and AI auto-calling remain Phase 13/restricted |

The optional DOB, anniversary and arbitrary profile-field request was not added. It requires a
cross-product custom-field/privacy decision and is mapped to the future customer-profile/configuration
work, not Phase 4. The existing Contact/Lead model already supports long-lived canonical identity and
repeat opportunities without a duplicate customer record.

## 2026-08-08 post-Phase-5 architecture amendment analysis

| Amendment request                                    | Existing state                                                                                                                            | Reconciled decision                                                                                                                                                                              | Implementation status                                                                                  |
| ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------ |
| One Unified Inbox across approved messaging channels | Phase 5 already used channel-neutral Conversation/Message records but earlier mapping grouped WhatsApp, SMS and email                     | Keep one canonical inbox, customer context, timeline and Conversation Owner. Provider-specific identities, credentials, webhooks, templates and policy rules remain behind adapters              | Documented in `DESIGN.md` and ADR-0037; Phase 5 screens continue to use the shared inbox               |
| Phase 5 channel scope                                | The older PRD appendix mentioned SMS/email adapter work in Phase 5                                                                        | The final amendment supersedes that mapping: Phase 5 activates official WhatsApp Business Platform only. SMS/email are deferred, and Instagram Direct/Facebook Messenger belong to Phase 13      | WhatsApp Cloud boundary implemented; all other provider sends fail closed or remain absent             |
| Shared transient client state                        | Mobile already used Zustand for authentication/platform state; inbox composer state was component-local and web did not depend on Zustand | Use separate, feature-scoped, non-persisted web/mobile Zustand stores for shared inbox UI workflow only. API/TanStack Query remains server truth; URL owns shareable web inbox selection/filters | Implemented with context-change reset wiring and web/mobile tests; ADR-0038                            |
| Sensitive-state protection                           | Credentials were backend-only, while no explicit store policy existed                                                                     | Never persist tokens, secrets, message bodies, documents, signed URLs, customer/location payloads or durable offline commands in ordinary Zustand stores                                         | `DESIGN.md`, prompts 05–14 and master audit updated; current inbox stores contain no secret/PII fields |
| Future channel expansion                             | Phase 13 listed provider integrations but did not explicitly prohibit parallel inbox products                                             | Instagram Direct and Facebook Messenger must extend the Phase 5 core; official APIs only. SMS/email may use the core where their product rules fit                                               | Deferred to Phase 13; no future channel implementation added during this amendment                     |

This amendment changes architecture and Phase 5 source behavior but creates no database migration.
The existing generic channel enum is an extension boundary, not evidence that SMS or email is live.
